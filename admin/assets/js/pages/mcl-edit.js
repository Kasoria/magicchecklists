...
import { ImageManager } from '../../../../public/assets/js/modules/mcl-image-manager.js';
...
document.addEventListener('DOMContentLoaded', function() {
    const EditChecklist = {
        ...
        initImageManager() {
            if (!this.imageManager) {
                const config = {
                    ...
                    drawer: document,
                    drawerContent: document.querySelector('.mcl-wrap'),
                    checklistId: document.querySelector('input[name="checklist_id"]')?.value
                };
        ...
        initItemsHandling() {
            ...
            // Add parent select handling
            const itemsList = document.getElementById('mcl-items');
            if (itemsList) {
                itemsList.addEventListener('change', (e) => {
                    if (e.target.matches('.mcl-parent-select')) {
        ...
        createListItem() {
            ...
            const itemId = `item_${Date.now()}_${this.itemIndex}`;
            li.setAttribute('data-item-id', itemId);
            
            li.innerHTML = `
                <span class="drag-handle">☰</span>
                <input type="hidden" name="items[${this.itemIndex}][id]" value="${itemId}">
                <div class="mcl-item-wrapper">
                    ...
                    <div class="mcl-item-controls">
                        <select class="mcl-parent-select" 
                                name="items[${this.itemIndex}][parent_id]"
                                data-item-id="${itemId}">
                            <option value="">${this.escapeHtml('No Parent')}</option>
                        </select>
                        ${this.getPrioritySelectHtml(this.itemIndex)}
                    </div>
                </div>
            ...
            // Set default priority to 'none' for new items
            const prioritySelect = li.querySelector('.mcl-priority-select');
            if (prioritySelect) {
            ...
            items.forEach((item, index) => {
                ...
                if (enabled) {
                    if (!existingSelect) {
                        ...
                        if (actionsContainer) {
                            actionsContainer.parentNode.insertBefore(select, actionsContainer);
                        }
                    }
                } else {
                    if (existingSelect) {
        ...
        initSortable() {
            ...
            new Sortable(itemsContainer, {
                ...
                onStart: (evt) => {
                    const item = evt.item;
                    // Add a nesting indicator div
                    const indicator = document.createElement('div');
                    indicator.className = 'mcl-nesting-indicator';
                    item.appendChild(indicator);

                    // If this item is a parent, hide its children during the drag
                    if (!item.hasAttribute('data-parent-id')) {
                        const itemId = item.dataset.itemId;
                        const children = Array.from(itemsContainer.querySelectorAll(`li[data-parent-id="${itemId}"]`));
                ...
                onMove: (evt, originalEvent) => {
                    ...
                    // If the dragged item is already a child, use a different threshold
                    if (dragged.hasAttribute('data-parent-id')) {
                        // Look up its current parent
                        const parentId = dragged.getAttribute('data-parent-id');
                        const parentItem = document.querySelector(`li[data-item-id="${parentId}"]`);
                        if (parentItem) {
                            const parentRect = parentItem.getBoundingClientRect();
                            // Define a "unnest" threshold for child items (e.g. parent's left + 20px)
                            const unnestThreshold = parentRect.left + 20;
                            if (mouseX < unnestThreshold) {
                                // The dragged child is being dragged far left – remove its parent.
                                dragged.removeAttribute('data-parent-id');
                                dragged.classList.remove('mcl-child-item');
                                const parentSelect = dragged.querySelector('.mcl-parent-select');
                                if (parentSelect) {
                                    parentSelect.value = '';
                                }
                            }
                            // Otherwise, if the dragged item is already a child and the mouse is not far left,
                            // we simply allow vertical reordering among its siblings without changing its parent.
                            this.updateParentSelects();
                            return true;
                        }
                    } else {
                        // For items that are not yet children, use the standard (nesting) threshold.
                        ...
                        // Do not allow nesting if the drop target (related item) is itself a child.
                        ...
                        if (isNesting) {
                            ...
                            const parentSelect = dragged.querySelector('.mcl-parent-select');
                            if (parentSelect) {
                                parentSelect.value = related.dataset.itemId;
                            }
                        } else {
                            // Ensure it remains a top‑level item.
                            dragged.removeAttribute('data-parent-id');
                            dragged.classList.remove('mcl-child-item');
                            const parentSelect = dragged.querySelector('.mcl-parent-select');
                            if (parentSelect) {
                ...
                onEnd: (evt) => {
                    ...
                    if (item.dataset.hasChildren === 'true') {
                        ...
                        const children = Array.from(itemsContainer.querySelectorAll('.mcl-hidden-child'));
                    ...
                    // Reattach any children that may have moved indirectly and update indexes
                    this.reattachAllChildren();
                    this.updateAllItemIndexes();
                    this.updateParentSelects();
                    this.hasUnsavedChanges = true;
                }
            });
        ...
        // Update the reattachAllChildren method to handle the new nesting
        reattachAllChildren() {
            const itemsContainer = document.getElementById('mcl-items');
            if (!itemsContainer) return;
            ...
            const children = Array.from(itemsContainer.querySelectorAll('li[data-parent-id]'));
            children.forEach(child => {
            ...
            parents.forEach(parentId => {
                const parentItem = itemsContainer.querySelector(`li[data-item-id="${parentId}"]`);
                if (parentItem) {
            ...
            // Update the parent selects to reflect the new structure
            this.updateParentSelects();
        },

        // Add new method to update all item indexes
        updateAllItemIndexes() {
            const items = document.querySelectorAll('#mcl-items li');
            let index = 0;
            
            items.forEach(item => {
                // Update hidden input index
                const hiddenInput = item.querySelector('input[type="hidden"]');
                if (hiddenInput) {
                    hiddenInput.name = `items[${index}][id]`;
                }
                
                // Update content field index
                const content = item.querySelector('.mcl-item-content');
                if (content) {
                    content.dataset.name = `items[${index}][content]`;
                }
                
                // Update parent select index
                const parentSelect = item.querySelector('.mcl-parent-select');
                if (parentSelect) {
                    parentSelect.name = `items[${index}][parent_id]`;
                }
                
                // Update priority select index if it exists
        ...
        initFormValidation() {
            ...
            // Form submission and submit button click
            const handleSubmit = (e) => {
                if (e) e.preventDefault();
                
                // Process items before validation
                this.processItemsBeforeSubmit();
                
                // Remove any existing error styling before validation
                document.querySelectorAll('.mcl-input-error').forEach(el => {
                    el.classList.remove('mcl-input-error');
                });
                ...
                if (!title?.value.trim()) {
                    errors.push('Title is required (Step 1: Basic Settings)');
                ...
                const shortcutInput = document.getElementById('mcl_keyboard_shortcut');
                const triggerWrapper = shortcutTrigger?.closest('.mcl-checkbox-group');

                if ((!shortcutTrigger?.checked) && (!buttonTrigger?.checked)) {
                    errors.push('At least one trigger method must be selected (Step 2: Advanced Settings)');
                    triggerWrapper?.classList.add('mcl-input-error');
                }

                if (shortcutTrigger?.checked && (!shortcutInput?.value.trim())) {
                    errors.push('Keyboard shortcut is required when shortcut trigger is enabled (Step 2: Advanced Settings)');
                    shortcutInput?.classList.add('mcl-input-error');
                }

                // Checklist items validation
                const items = document.querySelectorAll('.mcl-item-content');
                const itemsList = document.getElementById('mcl-items');
                ...
                if (!hasValidItem) {
                    errors.push('At least one non-empty checklist item is required');
                    itemsList?.classList.add('mcl-input-error');
                }
                ...
                // If no errors, clear unsaved changes flag and submit
                this.hasUnsavedChanges = false;
                form.submit();
            };

            // Bind to all submit buttons
            document.querySelectorAll('.mcl-submit-form').forEach(button => {
        ...
        displayValidationErrors(errors) {
            ...
            const existingErrorContainer = document.querySelector('.mcl-validation-errors');
            ...
            // Create error container above the steps
            const errorContainer = document.createElement('div');
            errorContainer.className = 'mcl-validation-errors';
            ...
            header.className = 'mcl-error-header';
            header.innerHTML = `
            ...
            // Insert error container before the progress tracker
            const progressTracker = document.querySelector('.mcl-progress-tracker');
            if (progressTracker) {
                progressTracker.parentNode.insertBefore(errorContainer, progressTracker);
            }
            ...
            if (errors.some(error => error.includes('Step 1'))) {
                this.goToStep(1);
            } else if (errors.some(error => error.includes('Step 2'))) {
                this.goToStep(2);
            }
        },
        ...
        validateForm() {
            ...
            if (!title?.value.trim()) {
                ...
                title?.classList.remove('mcl-input-error');
            }

            // Description validation - optional, just remove error if exists
            const description = document.getElementById('mcl_description');
            description?.classList.remove('mcl-input-error');

            // Drawer Trigger Method validation
            const shortcutTrigger = document.getElementById('mcl_trigger_shortcut');
            const buttonTrigger = document.getElementById('mcl_trigger_button');
            const triggerWrapper = shortcutTrigger?.closest('.mcl-checkbox-group');
            ...
            if ((!shortcutTrigger?.checked) && (!buttonTrigger?.checked)) {
                ...
                triggerWrapper?.classList.remove('mcl-input-error');
            }

            // Keyboard shortcut validation - only if shortcut trigger is enabled
            const shortcutInput = document.getElementById('mcl_keyboard_shortcut');
            if (shortcutTrigger?.checked && (!shortcutInput?.value.trim())) {
                ...
                shortcutInput?.classList.remove('mcl-input-error');
            }

            // Checklist items validation - at least one non-empty item
            const items = document.querySelectorAll('.mcl-item-content');
            const itemsList = document.getElementById('mcl-items');
            let hasValidItem = false;
            ...
            if (!hasValidItem) {
                ...
                itemsList?.classList.remove('mcl-input-error');
            }
        ...
        // Helper method to validate shortcut input
        validateShortcutInput() {
            const shortcutTrigger = document.getElementById('mcl_trigger_shortcut');
            const shortcutInput = document.getElementById('mcl_keyboard_shortcut');
            
            if (shortcutTrigger?.checked && shortcutInput) {
                const isValid = shortcutInput.value.trim() !== '';
                shortcutInput.classList.toggle('mcl-input-error', !isValid);
                this.updateFieldError(shortcutInput, isValid ? '' : 'Keyboard shortcut is required when shortcut trigger is enabled');
            }
        },

        // Helper method to update field error messages
        updateFieldError(element, errorMessage) {
            if (!element) return;
            
            // Remove existing error message
            const existingError = element.parentNode.querySelector('.mcl-field-error');
            if (existingError) {
            ...
            if (errorMessage) {
                ...
                errorDiv.className = 'mcl-field-error';
                errorDiv.textContent = errorMessage;
                element.parentNode.appendChild(errorDiv);
            }
        },

        handleParentChange(select) {
            const listItem = select.closest('li');
            const parentId = select.value;
            
            // Remove previous parent-child relationship
            listItem.classList.remove('mcl-child-item');
            listItem.removeAttribute('data-parent-id');
            
            if (parentId) {
                const parentItem = document.querySelector(`li[data-item-id="${parentId}"]`);
                if (parentItem) {
                    // First move the item to its new position
                    const lastChild = this.getLastChildOf(parentId);
                    if (lastChild) {
                        // Place after the last child
                        lastChild.after(listItem);
                    } else {
                        // Place directly after parent
                        parentItem.after(listItem);
                    }
                    
                    // Force a reflow before adding the visual classes
                    void listItem.offsetHeight;
                    
                    // Then add child classes and attributes
                    listItem.classList.add('mcl-child-item');
                    listItem.setAttribute('data-parent-id', parentId);
                }
            }
            
            // Update parent select options in all items
            this.updateParentSelects();
        ...
        updateParentSelects() {
            const items = document.querySelectorAll('#mcl-items li');
            const parentSelects = document.querySelectorAll('.mcl-parent-select');
            
            // Create a map of items that are already children
            const childItems = new Set();
            parentSelects.forEach(select => {
                if (select.value) {
                    childItems.add(select.closest('li').dataset.itemId);
                }
            });
            
            // Update each parent select
            parentSelects.forEach(select => {
                const currentItemId = select.dataset.itemId;
                const currentValue = select.value;
                
                // Clear existing options except first (No Parent)
                while (select.options.length > 1) {
                    select.remove(1);
                }
                
                // Add options for valid parent items
                items.forEach(item => {
                    const itemId = item.dataset.itemId;
                    const content = item.querySelector('.mcl-item-content').textContent;
                    
                    // Skip if item is self or is already a child
                    if (itemId === currentItemId || childItems.has(itemId)) {
                        return;
                    }
        ...
        // Add helper method to find the last child of a parent
        getLastChildOf(parentId) {
            const children = Array.from(document.querySelectorAll(`li[data-parent-id="${parentId}"]`));
        ...
        // Add this method to the EditChecklist object
        getPrioritySelectHtml(index) {
            const enableItemPriority = document.getElementById('mcl_enable_item_priority')?.checked;
            ...
            return `
                <select name="items[${index}][priority]" class="mcl-select mcl-priority-select">
                    ${Object.entries(priorityLevels)
                        .map(([value, label]) => `<option value="${value}">${label}</option>`)
                        .join('')}
                </select>
...
document.addEventListener('DOMContentLoaded', function() {
    const CustomThemeHandler = {
        init() {
            this.themeSelect = document.getElementById('mcl_theme');
            this.customSettings = document.querySelector('.mcl-custom-theme-settings');
            
            if (!this.themeSelect || !this.customSettings) return;
            ...
            this.handleCheckboxStyleChange();
        },
        ...
        initColorPickers() {
            const colorPickers = this.customSettings.querySelectorAll('.mcl-color-picker');
            
            if (jQuery?.fn?.wpColorPicker) {
                jQuery(colorPickers).wpColorPicker({
                    change: (event, ui) => {
                        // Trigger change event for form dirty state
                        const changeEvent = new Event('change', { bubbles: true });
                        event.target.dispatchEvent(changeEvent);
                    }
                });
        ...
        bindEvents() {
            ...
            this.customSettings.querySelectorAll('input[type="number"]').forEach(input => {
        ...
        handleThemeChange() {
            ...
            if (isCustom) {
                // Ensure all color pickers are properly initialized
                this.initColorPickers();
            }
        },
        ...
        initMediaUploader() {
            const uploadBtn = this.customSettings.querySelector('.mcl-media-upload-btn');
            const removeBtn = this.customSettings.querySelector('.mcl-media-remove-btn');
            const previewDiv = this.customSettings.querySelector('.mcl-media-preview');
            const hiddenInput = this.customSettings.querySelector('#mcl_checkbox_custom_icon');
            ...
            uploadBtn.addEventListener('click', (e) => {
                ...
                mediaUploader.on('select', function() {
                    ...
                    previewDiv.innerHTML = `<img src="${attachment.url}" alt="Custom checkmark">`;
        ...
        handleCheckboxStyleChange() {
            const styleSelect = this.customSettings.querySelector('#mcl_checkbox_style');
            const customSettings = this.customSettings.querySelector('.mcl-custom-checkbox-settings');
            const standardSettings = this.customSettings.querySelector('.mcl-standard-checkbox-settings');
            
            if (!styleSelect || !customSettings || !standardSettings) return;
            
            styleSelect.addEventListener('change', () => {
                const isCustom = styleSelect.value === 'custom';
                customSettings.style.display = isCustom ? 'block' : 'none';
                standardSettings.style.display = isCustom ? 'none' : 'block';
            });
    ...
    CustomThemeHandler.init();
});

// Add this CSS class for hiding children during drag
const style = document.createElement('style');
style.textContent = `
    .mcl-hidden-child {
