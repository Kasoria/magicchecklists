...
class MCL_Admin {
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            'mcl_checklists',
            __( 'Import Checklist', 'magic-checklists' ),
            __( 'Import', 'magic-checklists' ),
            'manage_options',
            'mcl_import',
    ...
    private function clean_page_slug($slug) {
    ...
    private function group_admin_pages($pages) {
        $groups = array(
            'Core' => array(),
            'Plugins' => array(),
            'Settings' => array(),
    ...
    public function toggle_active() {
        ...
        if ($active) {
            ...
            if (count($active_checklists) >= 10) {
                wp_send_json_error(array(
                    'message' => __('Cannot activate checklist: Maximum of 10 active checklists allowed.', 'magic-checklists')
                ));
                return;
            }
        }
    ...
    /**
     * Check if shortcode is enabled for a checklist
     * 
     * @param int $checklist_id The checklist ID
     * @return bool
     */
